...
Place it under:
    ...
    scripts/run_supplementary_runtime_co2_v2.py
...
def run_measurements(args: argparse.Namespace) -> int:
    ...
    for idx, spec in enumerate(specs, start=1):
        ...
        if args.dry_run:
            ...
            print(f"    -> FAILED: elapsed={elapsed:.2f}s. Metrics were left blank because the run failed.")
            if args.show_error_tail:
                tail = tail_text(log_path, int(args.show_error_tail))
                if tail:
                    print("    ---- log tail ----")
