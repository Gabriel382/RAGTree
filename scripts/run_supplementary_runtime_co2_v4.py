...
Place it under:
    ...
    scripts/run_supplementary_runtime_co2_v4.py
...
def iter_eligible_docs(path: Path, doc_filter_value: str) -> Iterable[Tuple[int, Dict[str, Any]]]:
    """Yield eligible JSONL documents together with their eligible index.

    The eligible index is the same index expected by the original RAGTree
    skip/limit semantics after applying the notebook doc-type filter. For
    example, with ``doc_filter_value="dev"``, index 0 means the first dev
    document, not necessarily the first physical line in the JSONL file.
    ...
    path:
        Input JSONL file.
    doc_filter_value:
        Notebook-style filter, usually "all" or "dev".

    Yields
    ------
    Tuple[int, Dict[str, Any]]
        ``(eligible_index, document)`` for every document that satisfies the
        filter.
    """
    ...
    with path.open("r", encoding="utf-8") as fin:
        for line_no, line in enumerate(fin, start=1):
            ...
            if not doc_type_matches(doc, doc_filter):
                continue

            yield eligible_index, doc
            eligible_index += 1
...
def metric_keys() -> List[str]:
    """Metric columns written for successful executions."""
    ...
    return [
        ...
        "estimated_total_energy_kwh",
        "estimated_total_co2_kg",
        "estimated_total_co2_g",
    ]


def empty_energy_and_co2() -> Dict[str, Optional[float]]:
    """Return blank metric values for dry runs and failed executions.

    This prevents a failed 1-second connection error from being reported as a
    meaningful runtime/CO2 estimate.
    """
...
def tail_text(path: Path, n_lines: int = 40) -> str:
    """Return the last n lines of a text file, used for readable failure diagnostics."""
...
def extract_error_summary(log_text: str, fallback: str = "") -> Tuple[str, str]:
    """Extract a compact error type/message from a traceback or process log.

    This is intentionally simple and robust: the most useful line in a Python
    traceback is usually the last non-empty line containing "Error:" or ending
    with "Exception: ...". If no such line is found, we return the last non-empty
    log line or the fallback string.
    """
    ...
    # Prefer the last Python exception-looking line.
    for ln in reversed(lines):
        if "Error:" in ln or "Exception:" in ln or ln.startswith(("ValueError:", "KeyError:", "TypeError:", "RuntimeError:", "FileNotFoundError:", "requests.exceptions.")):
            if ":" in ln:
                etype, msg = ln.split(":", 1)
...
def traceback_tail_from_text(log_text: str, n_lines: int = 20) -> str:
    """Return a compact tail from an arbitrary log/traceback string."""

    lines = (log_text or "").splitlines()
    return "\n".join(lines[-max(1, n_lines):])


def iter_candidate_eligible_docs(
    input_path: Path,
    doc_filter_value: str,
    *,
    start_index: int,
    ...
    """Yield candidate documents after filtering, starting at a given eligible index.
    ...
    input_path:
        JSONL file.
    doc_filter_value:
        Same filter used by the notebook command, e.g. "dev" or "all".
    start_index:
        First eligible index to try.
    max_attempts:
        Maximum number of candidate docs to yield. If <= 0, yield all remaining
        eligible docs.
    """
    ...
    yielded = 0
    for eligible_index, doc in iter_eligible_docs(input_path, doc_filter_value):
        if eligible_index < start_index:
            continue
        yield doc, eligible_index
        yielded += 1
        if max_attempts > 0 and yielded >= max_attempts:
            break

def preflight_vllm(selected_specs: Sequence[MethodSpec]) -> None:
    """Print a lightweight vLLM connectivity warning before long runs.

    The check is intentionally non-fatal. Some OpenAI-compatible servers may not
    expose /v1/models, but a connection-refused error is still useful to display
    before launching many method/dataset conditions.
    """

    needs_vllm = any(str(_arg_value(spec.args, "--backend", "")).lower() == "vllm" for spec in selected_specs)
    if not needs_vllm:
        return
    ...
    try:
        ...
        print("[supplementary] vLLM preflight skipped because requests is not importable.")
        return
    ...
    try:
        ...
        if resp.status_code < 500:
            print(f"[supplementary] vLLM preflight reached {url} with status {resp.status_code}.")
        else:
            print(f"[supplementary] vLLM preflight reached {url}, but server returned status {resp.status_code}.")
    except Exception as exc:
        print(
            "[supplementary] WARNING: could not reach the vLLM server at "
            f"{url}: {exc}. Runs using --backend vllm will probably fail quickly. "
            "Start vLLM, check VLLM_BASE_URL, or use another backend before the full run."
        )
...
def run_measurements(args: argparse.Namespace) -> int:
    ...
    print(f"[supplementary] sample_strategy={args.sample_strategy}")
    print(f"[supplementary] max_sample_attempts={args.max_sample_attempts}")

    if not args.dry_run and not args.skip_preflight_vllm:
        preflight_vllm(specs)
    ...
    for idx, spec in enumerate(specs, start=1):
        ...
        try:
            ...
            if eligible_docs <= 0:
                raise ValueError(
                    f"No eligible documents found in {input_path} using filter {spec.doc_filter_value!r}."
                )
        except Exception as exc:
            failures += 1
            row = {
                ...
                "error_traceback_tail": traceback_tail_from_text(traceback.format_exc(), 20),
            ...
            row.update(empty_energy_and_co2())
            rows.append(row)
        ...
        # Candidate selection.
        # - fixed: one document only, exactly args.sample_index.
        # - first-success: try args.sample_index, then subsequent eligible docs until one succeeds.
        if args.sample_strategy == "fixed" or args.dry_run:
            max_attempts = 1
        else:
            max_attempts = int(args.max_sample_attempts)

        candidates = iter_candidate_eligible_docs(
            input_path,
        ...
        last_cmd_for_display: List[str] = []
        sample_doc: Optional[Dict[str, Any]] = None
        effective_skip = int(args.sample_index)

        for attempt_no, (candidate_doc, candidate_skip) in enumerate(candidates, start=1):
            sample_doc = candidate_doc
            effective_skip = candidate_skip
            sample_doc_id = document_identifier(candidate_doc)
            sample_doc_type = str(candidate_doc.get("type", ""))
            sample_doc_tokens = count_tokens_roughly(candidate_doc)

            suffix = "" if attempt_no == 1 else f"__attempt_{attempt_no:02d}"
            log_name = f"{idx:03d}_{safe_slug(spec.method)}__{safe_slug(spec.dataset)}{suffix}.log"
            ...
            last_cmd_for_display = cmd_for_display
            ...
            if attempt_no == 1:
                ...
                print(
                    f"    -> retry attempt {attempt_no}/{max_attempts if max_attempts > 0 else 'all'}: "
                    f"sample_skip={effective_skip} | doc_id={sample_doc_id or 'N/A'}"
                )
            ...
            error_type, error_message = extract_error_summary(log_text, fallback=error or "")
            error_tail = traceback_tail_from_text(log_text, int(args.error_traceback_lines))
            ...
            last_error = error or error_message or f"Run failed. See log: {log_path}"
            last_error_type = error_type
            last_error_message = error_message
            last_error_tail = error_tail

            print(
                f"    -> failed attempt {attempt_no}: elapsed={elapsed:.2f}s, "
                f"error={last_error_type or 'unknown'}: {last_error_message[:300]}"
            )

            if args.sample_strategy == "fixed":
                break
        ...
        if sample_doc is None:
            ...
            row = {
                ...
                "success": False,
                "return_code": 1,
                "error": "No candidate sample document could be selected.",
                "error_type": "ValueError",
                "error_message": "No candidate sample document could be selected.",
        ...
        row = {
            ...
            "error_type": "" if success else last_error_type,
            "error_message": "" if success else last_error_message,
            "error_traceback_tail": "" if success else last_error_tail,
            "method_family": spec.method_family,
            "method": spec.method,
            "dataset": spec.dataset,
            "command_dataset_key": command_dataset_key,
            ...
            "failed_attempts": sum(1 for a in attempt_summaries if not a["success"]),
            ...
            "log_path": str(last_log_path or ""),
            "notebook_source": spec.notebook_source,
            "note": spec.note,
            "command": " ".join(last_cmd_for_display),
            "notebook_like_command": spec.notebook_like_command,
            "created_at_utc": now_utc_iso(),
        }
        row.update(energy)
        ...
        if args.dry_run:
            ...
            print(f"    -> FAILED: elapsed={elapsed:.2f}s. Metrics were left blank because all sample attempts failed.")
            if last_error_type or last_error_message:
                print(f"    error: {last_error_type}: {last_error_message}")
            if args.show_error_tail and last_log_path:
                tail = tail_text(last_log_path, int(args.show_error_tail))
                if tail:
                    print("    ---- log tail ----")
        ...
        payload = {
            "metadata": {
                ...
                "dry_run": bool(args.dry_run),
                "interpretation": (
                    "One successful eligible document is measured per method/dataset and multiplied by the number "
                    "of eligible JSONL documents. Values are approximate cost indicators, not exact "
                    "full-benchmark measurements. If sample_strategy=first-success, failed candidate samples are "
                    "reported in attempt_summaries_json and are not used for cost extrapolation."
                ),
            },
            "rows": rows,
        }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--sample-strategy",
        choices=["fixed", "first-success"],
        default="first-success",
        help=(
            "Document sampling strategy. 'fixed' measures only --sample-index. "
            "'first-success' starts at --sample-index and tries subsequent eligible documents until one succeeds. "
            "Default: first-success."
        ),
    )
    p.add_argument(
        ...
        default=20,
        help=(
            "Maximum candidate documents to try per method/dataset when --sample-strategy first-success. "
            "Use 0 to try all remaining eligible documents. Default: 20."
        ),
    )
    p.add_argument(
        ...
        type=int,
        default=20,
        help="Number of traceback/log lines to store in CSV/JSON when a run fails. Default: 20.",
    )
    p.add_argument(
        "--only-method",
        default=None,
    ...
    p.add_argument(
        ...
        type=int,
        default=40,
        help="Print the last N log lines when a run fails. Use 0 to disable. Default: 40.",
    )
    p.add_argument(
        "--skip-preflight-vllm",
        action="store_true",
        help="Skip the lightweight vLLM connectivity warning before non-dry runs.",
    )
    p.add_argument(
        "--stop-on-error",
        action="store_true",
