...
class TripleKGRetriever:
    """
    Reusa seu KG salvo pelo build_kg_from_preprocessed.py (LocalGraphStore):
      - seeds = entity IDs do doc
      - BFS até max_hops usando adjacências internas do LocalGraphStore
      - rank por token overlap doc<->triple (+ bonus para seeds)
    """
    ...
    def retrieve(self, doc: Dict[str, Any]) -> SimpleKGFragment:
        ...
        # BFS collecting candidate edges
        seen_nodes: Set[str] = set(seeds)
        frontier: Set[str] = set(seeds)
        seen_edge_ids: Set[int] = set()

        hops = max(1, int(self.params.max_hops))
        for _ in range(hops):
        ...
        for eid in seen_edge_ids:
            ...
            scored.append({"h": h, "r": r, "t": t, "score": score, "meta": meta})
